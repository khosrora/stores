...
import { checkConnection, createIndex } from '@products/elasticsearch';
...
function htppServer(httpServer: http.Server) {
  try {
    logger.info(`products Server has started with process id ${process.pid}`);
    httpServer.listen(port, () => {
      logger.info(`products Server running on port ${port}`);
    });
  } catch (error) {

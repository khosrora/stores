...
const checkConnection = async (): Promise<void> => {
  ...
  try {
    ...
    logger.info(`order service elastic search health status - ${health.status}`);
    isConnected = true;
  } catch (error) {
    logger.error('connection to Elastic search faild .. retrying ...');
    logger.log('error', 'order service checkConnection() : ', error);
    process.exit(-1);
  }
};
...
const createIndex = async (index: string) => {
  try {
    ...
    logger.log('error', 'order service createIndex() : ', error);
  }
};
